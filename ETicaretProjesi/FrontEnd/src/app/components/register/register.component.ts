...
@Component({
  ...
  template: `
    <div class="auth-page">
      <div class="auth-card">
        <h2>📋 Kayıt Ol</h2>
        <p class="subtitle">Alışverişe başlamak için hesap oluşturun</p>
        <div class="form-group">
          <label>Kullanıcı Adı</label>
          <input type="text" [(ngModel)]="username" placeholder="Kullanıcı adınızı girin" class="form-input" />
        </div>
        <div class="form-group">
          <label>📞 Telefon Numarası</label>
          <input type="tel" [(ngModel)]="phoneNumber" placeholder="05XX XXX XX XX" class="form-input" />
        </div>
        <div class="form-group">
          <label>Şifre</label>
          <input type="password" [(ngModel)]="password" placeholder="Şifrenizi girin" class="form-input" />
        </div>
        <div class="form-group">
          <label>Şifre Tekrar</label>
          <input type="password" [(ngModel)]="confirmPassword" placeholder="Şifrenizi tekrar girin" class="form-input" />
        </div>
        ...
        <p class="form-footer">Zaten hesabınız var mı? <a routerLink="/login">Giriş Yap</a></p>
      </div>
    </div>
  `,
  styles: [`
...
export class RegisterComponent {
  ...
  register() {
    ...
    this.loading = true;
    this.authService.register(this.username, this.password, this.phoneNumber).subscribe({
      next: res => {
        this.loading = false;
        if (res.success) {
      ...
      error: () => { this.loading = false; this.message = 'Bir hata oluştu.'; this.success = false; }
    });
  }
}
